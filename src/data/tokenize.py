...
def tokenize_pet_mlm_copa_txt(tokenizer, config, txt1, txt2, txt3, txt_trim, set_stop_words_ids, unsup_mask_idx=None):
    '''
    Tokenizes the text by trimming the appropriate txt
    ...
    txt1 = txt1.replace(" so ", "[MASK]").replace(" because ", "[MASK]")
    txt2 = txt2.replace(" so ", "[MASK]").replace(" because ", "[MASK]")
    txt3 = txt3.replace(" so ", "[MASK]").replace(" because ", "[MASK]")

    unsup_txt1_input_ids = tokenizer(txt1, add_special_tokens=False)["input_ids"]
    unsup_txt2_input_ids = tokenizer(txt2, add_special_tokens=False)["input_ids"]
    unsup_txt3_input_ids = tokenizer(txt3, add_special_tokens=False)["input_ids"]
    ...
    if tot_length <= config.max_text_length:
        ...
        unsup_masked_ids = [tokenizer.pad_token_id] * config.max_text_length
        unsup_masked_ids[:tot_length] = unsup_txt1_input_ids + unsup_txt2_input_ids + unsup_txt3_input_ids
    ...
    # Trim text
        ...
        if txt_trim == 0:
            new_txt1_input_ids = txt1_input_ids[:-num_trim]
            new_txt1_masked_input_ids = np.copy(new_txt1_input_ids)
            trunc_input_ids = new_txt1_input_ids + txt2_input_ids + txt3_input_ids
            unsup_masked_ids = new_txt1_masked_input_ids + txt2_input_ids + txt3_input_ids
        elif txt_trim == 1:
            new_txt2_input_ids = txt2_input_ids[:-num_trim]
            new_txt2_masked_input_ids = np.copy(new_txt2_input_ids)
            trunc_input_ids = txt1_input_ids + new_txt2_input_ids + txt3_input_ids
            unsup_masked_ids = txt1_input_ids + new_txt2_masked_input_ids + txt3_input_ids
        elif txt_trim == 2:
            new_txt3_input_ids = txt3_input_ids[:-num_trim]
            new_txt3_masked_input_ids = np.copy(new_txt3_input_ids)
            trunc_input_ids = txt1_input_ids + txt2_input_ids + new_txt3_input_ids
            unsup_masked_ids = txt1_input_ids + txt2_input_ids + new_txt3_masked_input_ids
        else:
            raise ValueError("Invalid Txt Trim")

    unsup_mask_idx = [unsup_masked_ids.index(tokenizer.mask_token_id)]

    trunc_input_ids = [tokenizer.cls_token_id] + trunc_input_ids
    unsup_masked_ids = [tokenizer.cls_token_id] + unsup_masked_ids

    return trunc_input_ids, unsup_masked_ids, unsup_mask_idx
